...
def build_vocab(file_path, tokenizer, max_size, min_freq):
    ...
    with open(file_path, 'r', encoding='UTF-8') as f:
        ...
        for line in tqdm(f):
            ...
            for word in tokenizer(content):#分词 or 分字,构建词或字到频数的映射 统计词频/字频
...
def build_dataset(config, ues_word):
    # 定义分词/字函数tokenizer,分为（word-level/character-level）
    ...
    print("Vocab size: {len(vocab)}") #输出词/字典大小
    ...
    def load_dataset(path, pad_size=32):
        ...
        with open(path, 'r', encoding='UTF-8') as f:
            for line in tqdm(f):#遍历每一行
                ...
                if not lin:#遇到空行 跳过
                    continue
                content, label = lin.split('\t')#每一行以\t为切分
                words_line = []
                token = tokenizer(content)#对文本进行分词/分字
                seq_len = len(token) #序列/文本真实长度（填充或截断前）
                if pad_size:#对分词进行长截短填
                    if len(token) < pad_size:#文本真实长度比填充长度短
    ...
    return vocab, train, dev, test#返回字/词典 预处理好的训练集、验证集、测试集
...
class DatasetIterater(object):#自定义数据集迭代器
    ...
    def __next__(self):
        if self.residue and self.index == self.n_batches:#当数据集大小不整除 batch_size时，构建最后一个batch
            ...
            batches = self.batches[self.index * self.batch_size: (self.index + 1) * self.batch_size]#把当前batch转换为tensor 并 to(device)
            self.index += 1
